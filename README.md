...
- **Giao diện người dùng hiện đại (FlashMessage)**: Thay thế hoàn toàn các hộp thoại thông báo cũ kỹ (alert/confirm) bằng hệ thống thông báo Toast chuyên nghiệp, không gây gián đoạn trải nghiệm (Non-blocking UI).
- **Kiểm soát tính toàn vẹn dữ liệu**: Ràng buộc chặt chẽ các quy trình nghiệp vụ (ví dụ: chỉ cho phép khóa bảng điểm khi đã nhập đủ 3 đầu điểm CC, GK, CK).
- **Bảo mật & Phân quyền Coi thi**: Giảng viên chỉ có thể truy cập lịch thi cá nhân và các phòng thi được phân công cụ thể, đảm bảo tính riêng tư và bảo mật dữ liệu thi cử.
- **Việt hóa Toàn diện**: 100% thông báo lỗi hệ thống và phản hồi từ Server được chuẩn hóa sang Tiếng Việt.
...
Dự án vừa trải qua đợt nâng cấp lớn về UI/UX và Logic nghiệp vụ:
1.  **Hệ thống Thông báo mới**: Tích hợp `FlashMessageService` giúp giao diện phản ứng nhanh và tinh tế hơn.
2.  **Logic Điểm danh thông minh**: Tự động cảnh báo khi giảng viên quên lưu từ khóa hoặc chốt danh sách trống.
3.  **Bảo vệ dữ liệu Bảng điểm**: Ngăn chặn việc khóa sổ nếu sinh viên còn thiếu đầu điểm, tránh sai sót trong tính toán GPA.
4.  **Phân quyền Lịch coi thi**: Ẩn hoàn toàn dữ liệu phòng thi của đồng nghiệp, giảng viên chỉ thấy lịch của chính mình.
5.  **Chuẩn hóa Ngôn ngữ**: Dịch 100% các câu thông báo từ Backend sang Tiếng Việt.
